...
WHERE 
  ...
  order by 1;`);
	...
	// 	`select schemaname::text, sequencename::text, start_value::text, min_value::text, max_value::text, increment_by::text, cycle::text, cache_size::text from pg_sequences as seq${
	...
	// 		`select n.nspname::text as enum_schema,
	...
	//   order by n.nspname::text, t.typname::text, e.enumsortorder::text;`,
	// 	);
	...
	// >(
	// 	`SELECT rolname::text, rolinherit, rolcreatedb, rolcreaterole FROM pg_roles;`,
	// );
	...
	// 		withCheck: string;
	// 	}
	// >(`SELECT schemaname::text, tablename::text, policyname::text as name, permissive::text as "as", roles::text as to, cmd::text as for, qual::text as using, with_check::text as "withCheck" FROM pg_policies${
